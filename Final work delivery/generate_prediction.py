...
for imgIdx in range(5):
     ...
     image = ri.retinal_image(fitting_img[imgIdx], 'train')
...
for imgIdx in range(5):
    ...
    nr_pixels_to_classify = len(coords_vessel_pixels[0])
    X = np.zeros([nr_pixels_to_classify, nr_features])
    X=np.load(open('test' + str(imgIdx) +'.npy', 'rb'))
    ...
    for props in regions:
        ...
        if (nArteries>nVeins):
            ...
            predictions_image[coordinates_intersection_veins[0],coordinates_intersection_veins[1],2]=1-np.mean(predictions_image[coordinates_intersection_arteries[0],coordinates_intersection_arteries[1],0])
    
        else:
            #Find intersection between points which were predicted as veins and current region
            #At intersection region, veins are now arteries 
            predictions_image[coordinates_intersection_arteries[0],coordinates_intersection_arteries[1],2]=np.mean(predictions_image[coordinates_intersection_veins[0],coordinates_intersection_veins[1],2])
            #Predictions at arteries are now the mean of all veins probabilities 
            predictions_image[coordinates_intersection_arteries[0],coordinates_intersection_arteries[1],0]=1-np.mean(predictions_image[coordinates_intersection_veins[0],coordinates_intersection_veins[1],2])
...
#        neighbourhood=image.labels[rows_branch[jj]-3:rows_branch[jj]+3,cols_branch[jj]-3:cols_branch[jj]+3]
...
#        
#        for aa in range(len(labelsInNeighbourhood)):
#            label_coord=np.nonzero(image.labels==labelsInNeighbourhood[aa])
#            means_artery[aa]=np.mean(temp_image[label_coord[0],label_coord[1],0])
#            means_vein[aa]=1-np.mean(temp_image[label_coord[0],label_coord[1],0])
#
#    #See number of labels which are mainly artery or vein   
#        numbArteries=np.count_nonzero(means_vein>0.5)
#        numbVeins=np.count_nonzero(means_artery>0.5)
#        idx_vein=np.nonzero(means_vein>0.5)
#        idx_artery=np.nonzero(means_artery>0.5)
...
#                if (not np.isin(label_vein,modified_labels) and len(idx_artery[0])!=0 and len(idx_vein[0])!=0):
#                    predictions_image[label_vein_coord[0],label_vein_coord[1],0]=np.max(means_artery[idx_artery[0]])
#                    predictions_image[label_vein_coord[0],label_vein_coord[1],2]=1-np.max(means_artery[idx_artery[0]])
#                modified_labels.append(label_vein)
#
...
#                #Replace those arteries by veins
#                if (not np.isin(label_artery,modified_labels) and len(idx_artery[0])!=0 and len(idx_vein[0])!=0):
#                    predictions_image[label_artery_coord[0],label_artery_coord[1],2]=np.max(means_vein[idx_vein[0]])
#                    predictions_image[label_artery_coord[0],label_artery_coord[1],0]=1-np.max(means_vein[idx_vein[0]])
#                modified_labels.append(label_artery)
#
#
#            
#        
...
#    plt.show()
    ...
    coords_vessel_pixels_final = np.nonzero(image.vessels)
    vein_predictions=vesselsWithSkeletonPrediction[:,:,2]
    
    acc=metrics.accuracy_score(image.veins[coords_vessel_pixels_final],vein_predictions[coords_vessel_pixels_final]>0.5)
    ...
    fpr,tpr,th=metrics.roc_curve(image.veins[coords_vessel_pixels_final],vein_predictions[coords_vessel_pixels_final])
    f1_score=metrics.f1_score(image.veins[coords_vessel_pixels_final],vein_predictions[coords_vessel_pixels_final]>0.5)
    AUC=metrics.roc_auc_score(image.veins[coords_vessel_pixels_final],vein_predictions[coords_vessel_pixels_final])

    f1scores_vessel.append(f1_score)
    auc_vessel.append(AUC)
    print("acc on vessel:", acc)
    print("f1_score on vessel:",f1_score)
    print("AUC",AUC)
    print("acc_score vessel:",metrics.accuracy_score(image.veins[coords_vessel_pixels_final],vein_predictions[coords_vessel_pixels_final]>0.5))
    print("-------------------------------")
print("meanacc_vessel",np.mean(accuracies_vessel), "+/-",np.std(accuracies_vessel))
print("meanacc_skel",np.mean(accuracies_skeleton),"+/-",np.std(accuracies_skeleton))
print("meanf1_vessel", np.mean(f1scores_vessel),"+/-",np.std(f1scores_vessel))
print("meanAUC_vessel", np.mean(auc_vessel),"+/-",np.std(auc_vessel))
